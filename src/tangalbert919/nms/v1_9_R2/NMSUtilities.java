package tangalbert919.nms.v1_9_R2;
...
import tangalbert919.nms.v1_9_R2.anvil.CustomContainerAnvil;
import tangalbert919.nms.v1_9_R2.anvil.TileEntityCustomContainerAnvil;
import tangalbert919.nms.v1_9_R2.mobs.*;
...
public class NMSUtilities {
        ...
        if (str.indexOf("$0") >= 0 && str.indexOf("$1") >= 0) {
            ...
            for (int j = 0; j < i; ++j) {
                ...
                if (m == null) {
                    ...
                    m = (new JSONMessage(str.split("\\$0")[0])).then(ChatColor.getLastColors(str) + murdered_pre).itemTooltip(murdered.getItemOnCursor()).then(ChatColor.getLastColors(str) + str.split("\\$0")[1].split("\\$1")[0]).then(ChatColor.getLastColors(str) + murderer_pre).itemTooltip(murderer.getItemOnCursor()).then(ChatColor.getLastColors(str) + (str.split("\\$1").length > 1 ? str.split("\\$1")[1] : ""));
                    cache.put(l, m);
                }
    ...
    public static void spawnEntity(Location inLoc, EntityType type) {
        ...
        ((EntityInsentient) entity).prepare(world.D(new BlockPosition((Entity) entity)), (GroupDataEntity) null);
